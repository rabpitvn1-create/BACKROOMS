...
function cleanKey(value) { return typeof value === "string" ? value.trim().slice(0, 160) : ""; }
function numberFlag(state, key) { const value = Number(state?.flags?.[key]); return Number.isFinite(value) ? value : 0; }
function partyNames(state) {
  if (!Array.isArray(state?.party)) return [];
  return state.party.map((member) => typeof member === "string" ? member : member?.name)
    .filter((name) => typeof name === "string" && name.trim()).map((name) => name.trim().toLowerCase());
}
function newPartyMember(current, nextState) { const before = new Set(partyNames(current)); return partyNames(nextState).some((name) => !before.has(name)); }
...
function approveSnapshot(current, nextState, generated) {
  ...
  if (event.shouldGenerate !== true || !SNAPSHOT_TYPES.has(type)) return { requested: false, type: null, key: null, reason: null };
  const beforeLevel = levelFromState(current); const afterLevel = levelFromState(nextState);
  let eligible = false; let triggerKey = "";
  if (type === "level_transition") {
    ...
    const beforeCount = numberFlag(current, "entitiesConfirmedLocal"); const afterCount = numberFlag(nextState, "entitiesConfirmedLocal");
    const beforeEncounter = cleanKey(current?.flags?.entityEncounterKey); const afterEncounter = cleanKey(nextState?.flags?.entityEncounterKey);
    ...
    triggerKey = eligible ? `character:${partyNames(nextState).sort().join(",")}:${numberFlag(nextState, "survivorsConfirmed")}:${levelLabel(afterLevel) || "unknown"}` : "";
  } else if (type === "special_area") {
    const beforeArea = cleanKey(current?.flags?.visualAreaKey); const afterArea = cleanKey(nextState?.flags?.visualAreaKey);
    eligible = Boolean(afterArea && afterArea !== beforeArea); triggerKey = eligible ? `area:${levelLabel(afterLevel) || "unknown"}:${afterArea}` : "";
  } else if (type === "major_event") {
    const beforeEvent = cleanKey(current?.flags?.visualEventKey); const afterEvent = cleanKey(nextState?.flags?.visualEventKey);
    eligible = Boolean(afterEvent && afterEvent !== beforeEvent); triggerKey = eligible ? `event:${afterEvent}` : "";
  }
  const lastTrigger = cleanKey(current?.flags?.lastSnapshotTriggerKey);
  if (!eligible || !triggerKey || triggerKey === lastTrigger) return { requested: false, type: null, key: null, reason: null };
  return { requested: true, type, key: triggerKey.slice(0, 300), reason: typeof event.reason === "string" ? event.reason.trim().slice(0, 300) : "" };
}

function hardIssues(audits) {
  return audits.flatMap((audit) => Array.isArray(audit?.issues) ? audit.issues.filter((issue) => issue?.severity === "hard") : []);
}
function rejectedOperationIssues(operationResult) {
  if (!Array.isArray(operationResult?.rejected) || operationResult.rejected.length === 0) return [];
  return operationResult.rejected.slice(0, 12).map((entry) => ({
    rule: "state_narrative_mismatch",
    severity: "hard",
    claim: entry?.op?.type || "rejected operation",
    reason: `Backend rejected proposed state operation: ${entry?.reason || "unknown_reason"}. Rewrite the reply so it does not narrate that rejected state change, and omit the invalid operation.`,
  }));
}
...
async function auditsForRisk({ risk, current, action, rolls, generated, operationResult }) {
  ...
  const common = { current, action, rolls, generated, acceptedOps: operationResult.accepted, rejectedOps: operationResult.rejected,
    excludeSlots: Number.isInteger(writerSlot) ? [writerSlot] : [] };
  if (risk.level === AUDIT_LEVEL.NARROW) return [await auditTurn({ ...common, scope: "canon" })];
  return Promise.all([auditTurn({ ...common, scope: "canon" }), auditTurn({ ...common, scope: "character" })]);
}

function applyServerFields(current, operationResult, generated, gameplay, rolls) {
  let nextState = operationResult.state; const nextLevel = levelFromState(nextState);
  nextState = {
    ...nextState,
    ...
    flags: gameplay ? { ...objectOr(nextState.flags, current.flags), lastRolls: { turn: current.turn, ...rolls }, ...(nextLevel ? { currentLevel: nextLevel } : {}) } : current.flags,
    snapshotUrl: current.snapshotUrl,
    log: [...(Array.isArray(current.log) ? current.log : []), { role: "player", text: generated._action }, { role: "gm", text: generated.reply.trim() }],
  };
  return nextState;
}
...
export async function POST(request) {
  try {
    ...
    if (issues.length) {
      return json({
        error: "Lượt chơi không vượt qua kiểm tra canon/state; state không được thay đổi.",
        saved: false,
        storage: storageName(),
    ...
    const snapshot = gameplay ? approveSnapshot(current, nextState, generated) : { requested: false, type: null, key: null, reason: null };
    if (snapshot.requested) nextState.flags = { ...nextState.flags, lastSnapshotTriggerKey: snapshot.key };

    const state = await saveState(sessionId, nextState, current.revision);
    return json({
      state, storage: storageName(), saved: true, rolls, turnAdvanced: gameplay,
      operationsAccepted: operationResult.accepted.length,
      operationsRejected: operationResult.rejected.map((entry) => entry.reason),
      auditLevel: risk.level, auditCount: audits.length, auditRepaired: repaired,
      snapshotRequested: snapshot.requested, snapshotType: snapshot.type, snapshotReason: snapshot.reason,
    });
  } catch (error) {
    if (error instanceof StateConflictError) return json({ error: error.message, saved: false, storage: storageName() }, 409);
