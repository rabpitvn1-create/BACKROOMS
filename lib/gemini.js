...
QUY TẮC XỬ LÝ LƯỢT:
- State dưới đây là source of truth của phiên hiện tại. Không làm state lùi turn hoặc quay về Prologue.
- DICE SERVER nếu được cung cấp là bất biến. Không sửa raw, không reroll và không tự tạo một kết quả ngẫu nhiên khác để thay thế.
...
Chỉ thay đổi dữ liệu khi diễn biến của lượt thực sự làm nó thay đổi. Với object/array không thay đổi, trả lại dữ liệu hiện tại thay vì xóa mất dữ kiện canon.`;
  ...
  for (const key of keys) {
    try {
      ...
      lastError = error;
      // Chỉ chuyển sang key kế tiếp khi lỗi là quota/rate-limit hoặc lỗi tạm thời.
