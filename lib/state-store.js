...
import { CANON_VERSION, createCanonicalState } from "./canon.js";
...
export function defaultState(sessionId) {
  return createCanonicalState(sessionId);
}

export function normalizeState(input, sessionId, { keepRevision = false } = {}) {
  const source = input && typeof input === "object" ? input : {};
  const base = defaultState(sessionId);
  const turn = Number.isInteger(source.turn) && source.turn >= base.turn ? source.turn : base.turn;

  return {
    ...source,
    version: Number.isInteger(source.version) && source.version > 0 ? source.version : base.version,
    sessionId,
    title: cleanString(source.title, base.title, 200),
    turn,
    mode: cleanString(source.mode, base.mode, 64),
    canonLoaded: true,
    canonVersion: cleanString(source.canonVersion, CANON_VERSION, 120),
    ...
    flags: source.flags && typeof source.flags === "object" && !Array.isArray(source.flags) ? source.flags : base.flags,
    snapshotUrl: typeof source.snapshotUrl === "string" ? source.snapshotUrl.slice(0, 8000) : null,
    log: cleanLog(source.log).length ? cleanLog(source.log) : base.log,
...
export async function loadState(sessionId) {
  ...
  } else if (state.canonLoaded !== true) {
    // Placeholder/preview-era saves are not a real campaign. Replace them once with
    // the authoritative active run from Novel instead of carrying Turn 0 forward.
    state = defaultState(sessionId);
    await writeWithoutConflict(sessionId, state);
  }
