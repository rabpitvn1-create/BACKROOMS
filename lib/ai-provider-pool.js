...
async function requestGemini(worker, prompt) {
  let lastError;
  const health = stateFor(worker.slot);
  health.inFlight += 1;
  try {
...
async function requestLuna(prompt) {
...
export async function generateProviderText(prompt, options = {}) {
  const workers = orderedWorkers(options.excludeSlots || []);
  let lastError;
  ...
  for (const worker of workers) {
    ...
    try {
      const text = await requestGemini(worker, prompt);
  ...
  if (options.allowLuna !== false) {
    try {
      const text = await requestLuna(prompt);
