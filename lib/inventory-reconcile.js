...
const DIRECT_PATTERNS = [
  /(?<![\p{L}\p{N}_])(?:nhặt|lấy|cầm|thu hồi|tịch thu|nhận)\s+(?:(?:một|cái|chiếc|con|quyển)\s+)?(.{2,120}?)(?=\s+(?:lên|ra|rồi|và|để|bỏ|cất|đưa|vào|cho)\b|[,.;!?]|$)/iu,
...
export function applyTurnWithPickupReconcile(current, generated, action, rolls) {
  const candidate = extractConfirmedPickupCandidate(action, generated?.reply);
  ...
  if (!candidate || !isMundaneConfirmedPickupName(candidate)) return { ...result, ops: reconciledOps };
  ...
  for (const entry of result.rejected) {
    ...
    const eligible = !promoted
      ...
      && op?.basis === "gm_confirmed_pickup"
      && isMundaneConfirmedPickupName(pickupName)
      && tokenOverlap(pickupName, candidate);
    if (!eligible) {
      rejected.push(entry);
      continue;
    }

    if (!inventoryHasName(state, pickupName)) {
      const inventory = Array.isArray(state.inventory) ? [...state.inventory] : [];
      inventory.push({ ...op.item, name: String(pickupName).trim(), quantity: 1, state: op.item?.state || "STORED" });
