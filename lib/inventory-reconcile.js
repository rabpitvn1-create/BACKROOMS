...
const WEAK_PICKUP_CANDIDATE = /^(lên|ra|vào|xuống|lại|qua|đi|đến|nó|đó|up|out|in|down|back|it|that)$/iu;
...
function candidateFromAction(action) {
  ...
  const direct = candidateFromPatterns(text, DIRECT_PATTERNS);
  if (direct && !WEAK_PICKUP_CANDIDATE.test(direct)) return direct;
  return candidateFromPatterns(text, INTRO_PATTERNS);
}
...
function sameItemFuzzy(left, right) {
  return tokenOverlap(left, right) && tokenOverlap(right, left);
}

function findInventoryMatch(state, name) {
  const inventory = Array.isArray(state?.inventory) ? state.inventory : [];
  return inventory.find((item) => sameItemFuzzy(item?.name || item, name)) || null;
}
...
function desiredInventoryOutcome(current, generated, action) {
  ...
  if (restoreTarget && isMundaneConfirmedPickupName(restoreTarget) && replyConfirmsRestore(restoreTarget, reply)) {
    return { name: restoreTarget, source, basis: "omnivault_restore" };
  }
  const pickup = extractConfirmedPickupCandidate(action, reply);
  if (pickup && isMundaneConfirmedPickupName(pickup)) return { name: pickup, source: pickup, basis: "gm_confirmed_pickup" };
  return null;
}
...
export function reconcileConfirmedPickupOps(current, generated, action) {
  ...
  let existingOps = Array.isArray(generated?.ops) ? generated.ops.map((op) => ({ ...op })) : [];
  if (!outcome) return existingOps;

  if (outcome.basis === "omnivault_restore" && outcome.source && !sameItemFuzzy(outcome.source, outcome.name)) {
    existingOps = existingOps.filter((op) => {
      if (op?.type !== "inventory_upsert" || !op?.item?.name) return true;
      return !sameItemFuzzy(op.item.name, outcome.source);
    });
  }
...
export function applyTurnWithPickupReconcile(current, generated, action, rolls) {
  ...
  for (const entry of result.rejected) {
    ...
    const inventory = Array.isArray(state.inventory) ? [...state.inventory] : [];
    const index = inventory.findIndex((item) => sameItemFuzzy(item?.name || item, pickupName));
    if (index >= 0) {
