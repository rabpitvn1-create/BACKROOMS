...
main = replace_once(
    ...
    '  private static final String QWEN_DEFAULT_MODEL = "qwen3.6-plus";\n'
    '  private static final String OPENAI_MODEL = "gpt-5.4-mini";\n',
    "Qwen model constant",
)

qwen_method = r'''  private String qwenText(String prompt) throws Exception {
    if (BuildConfig.QWEN_API_KEY == null || BuildConfig.QWEN_API_KEY.trim().isEmpty()) {
      throw new HttpError(401, "Qwen API key chưa được cấu hình.");
    }
    String baseUrl = BuildConfig.QWEN_BASE_URL == null ? "" : BuildConfig.QWEN_BASE_URL.trim();
    if (baseUrl.isEmpty()) throw new Exception("Qwen Base URL chưa được cấu hình.");
    while (baseUrl.endsWith("/")) baseUrl = baseUrl.substring(0, baseUrl.length() - 1);

    String model = BuildConfig.QWEN_MODEL == null ? "" : BuildConfig.QWEN_MODEL.trim();
    if (model.isEmpty()) model = QWEN_DEFAULT_MODEL;
    ...
    if (text.isEmpty()) throw new Exception("Qwen không trả nội dung.");
    return text;
  }
...
new_generate = r'''  private String generateText(String prompt) throws Exception {
    ...
    try {
      ...
      throw new Exception("Qwen: " + qwenMessage + "; GPT fallback: " + gptMessage + "; Gemini fallback: " + geminiMessage);
    }
  }
'''
...
print("Game Master provider order: Qwen via secret Base URL -> GPT -> Gemini.")
