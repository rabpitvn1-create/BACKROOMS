...
# ---------------------------------------------------------------------------
# Lucia canon: keep the existing follower/loadout and add the requested passive.
# +5% means +5 percentage points on the existing generic `loot` roll while Lucia
# is actually in Party. No separate loot roll, pity roll, or inventory bypass.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Combat: a joint-attack order must resolve Lucia as a real combatant instead of
# silently treating the entire Party command as Kai-only. Lucia gets an independent
# normal hit roll using the same current hit chance as Kai, with her canonical M4A1
# gameplay damage (26) reduced by the target's existing armor. This does not alter
# Guilty Crown Override, Entity evasion, Diệp Minh, or Kai automatic skills.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Gameplay loot: augment the existing per-Level generic loot threshold by exactly
# 500 / 10000 = 5 percentage points while Lucia is in Party. The existing search
# eligibility gate and sanitizer remain authoritative.
# ---------------------------------------------------------------------------
...
if 'LUCIA SCOUT PASSIVE HARD LOCK:' not in main:
    ...
    # Healing-items runs before this final patch and already owns its directive.
    # Compose after it instead of rewriting or removing the existing prompt contracts.
...
# ---------------------------------------------------------------------------
# Avatar rebuild: strip mutable EXIF APP1 metadata while preserving the encoded
# JPEG image data and ICC/JFIF segments. This gives the APK a deterministic Lucia
# avatar build without inventing a new portrait or changing the visual itself.
# ---------------------------------------------------------------------------
def strip_exif_app1(raw: bytes) -> bytes:
    ...
    while i < len(raw):
        if raw[i] != 0xFF:
            # Entropy-coded scan data begins after SOS; keep it byte-for-byte.
            out.extend(raw[i:])
            break
        start = i
        ...
        marker = raw[i]
        i += 1
        if marker == 0xDA:  # Start of Scan: append the rest unchanged.
            out.extend(raw[start:])
            break
...
# ---------------------------------------------------------------------------
# Focused regression: joint orders must produce a Lucia combat resolution entry.
# Keep the assertion independent of hit/miss RNG; the bug was that Lucia was not
# resolved at all. Existing combat mechanics still decide whether her shot lands.
# ---------------------------------------------------------------------------
