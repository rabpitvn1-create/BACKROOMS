...
APK=${1:-Backroom-1.4.2-debug.apk}
BUILD_TOOLS=$(find "$ANDROID_HOME/build-tools" -mindepth 1 -maxdepth 1 -type d | sort -V | tail -1)

"$BUILD_TOOLS/apksigner" verify --verbose --print-certs "$APK"
"$BUILD_TOOLS/aapt" dump badging "$APK" | grep -q "package: name='com.rabpit.backroom'"
"$BUILD_TOOLS/aapt" dump badging "$APK" | grep -q "versionCode='97'"
"$BUILD_TOOLS/aapt" dump badging "$APK" | grep -q "versionName='1.4.2'"
...
# can loop the exact local resource without a network dependency. Also require the
# lifecycle helper name in DEX so a future patch-chain edit cannot silently package
# the song without actually wiring playback.
