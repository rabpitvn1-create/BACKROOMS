...
if len(raw) < 24 or raw[:8] != b"\x89PNG\r\n\x1a\n":
    raise RuntimeError("BestKai.png is not a valid PNG")
width = int.from_bytes(raw[16:20], "big")
height = int.from_bytes(raw[20:24], "big")
if width < 512 or height < 768:
    raise RuntimeError(f"BestKai.png is too small: {width}x{height}")

for path in (MAIN, INDEX):
    text = path.read_text(encoding="utf-8")
    text = text.replace("kai_snapshot_overlay.png", "BestKai.png")
    text = text.replace("kai_snapshot_overlay.webp", "BestKai.png")
...
if "BestKai.png" not in combined:
    raise RuntimeError("BestKai.png is not referenced by the finalized runtime")
