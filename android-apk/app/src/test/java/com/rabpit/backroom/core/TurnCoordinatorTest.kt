...
class TurnCoordinatorTest {
  @Test fun pendingTurnSurvivesAndCannotCommitTwice() {
    val created = TurnCoordinator.createPending(GameState.initial(), "TURN_184", "Kai nhặt chai nước")
    assertEquals("TURN_184", TurnCoordinator.recover(created.state)?.turnId)
    val command = ItemCommand("TURN_184:0", "TURN_184", KAI_ID, source = CommandSource.RULE,
    ...
    val retry = TurnCoordinator.createPending(committed.state, "TURN_184", "Kai nhặt chai nước")
  ...
  @Test fun failedBatchIsAtomic() {
    ...
    val pickup = ItemCommand("c1", "TURN_2", KAI_ID, source = CommandSource.RULE,
      operation = ItemCommand.Operation.PICKUP, itemId = "water", itemName = "Water")
    ...
    val result = TurnCoordinator.commit(created.state, listOf(pickup, invalid))
