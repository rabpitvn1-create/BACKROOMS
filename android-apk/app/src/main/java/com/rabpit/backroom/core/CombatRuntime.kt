...
object CombatRuntime {
  ...
  private const val JANE_DOE_EVASION_BP = 2500
  ...
  private val profiles = listOf(
    ...
    Profile(JOHN_DOE_KEY, "Jane Doe", JOHN_DOE_MAX_HP, 0, 6, 9),
  ...
  fun resolve(state: GameState, actionKind: String, action: String): Resolution {
    ...
    if (johnDoeTargetStunned) {
      ...
      log += "Jane Doe Stun: mục tiêu bị Stun và không thể thực hiện hành động trong lượt hiện tại."
    }
    ...
    if (c.entityKey == JOHN_DOE_KEY && !janeDoeLilith(resolvedState) && roll(c.copy(eventCounter = c.eventCounter + 1901), 100) < JANE_DOE_LILITH_PROC) {
    ...
    when (intent) {
      ...
      Intent.ATTACK -> {
        ...
        if (violetWardenKaiActionLocked) {
          ...
          log += if (entityEvaded) "${c.entityName} né đòn (${entityEvasionBp(resolvedState, c.entityKey) / 100.0}% evasion) và giành lại áp lực." else "Đòn đánh trượt; ${c.entityName} giành lại áp lực."
        }
    ...
    if (c.entityHp <= 0) {
      ...
      return Resolution(cleared, true, localizeCombatNarration(log.joinToString(" ")) + " ${c.entityName} đã bị tiêu diệt." + defeatLootNarration(cleared, c.encounterId), entityDestroyed = true)
    }
    ...
    if (c.entityKey == JOHN_DOE_KEY && johnDoePoisonedIds(resolvedState).isNotEmpty()) {
      ...
      log += "Poison Jane Doe: từng mục tiêu đang bị ảnh hưởng mất ${JOHN_DOE_POISON_DAMAGE_PERCENT}% Max HP riêng biệt. ${poison.summary}."
    }
    ...
    if (entityStunnedThisTurn) {
      ...
      entityTargets.forEachIndexed { actionIndex, targetId ->
        ...
        if (incomingRoll < enemyChance) {
          var requestedDamage = when (c.entityKey) {
            ...
            JOHN_DOE_KEY -> janeDoeDamage(resolvedState, targetMaxHp, JOHN_DOE_ATTACK_PERCENT)
          ...
          val legacyHit = when {
            ...
            targetId == KAI_ID && c.entityKey == JOHN_DOE_KEY ->
              "Jane Doe tấn công: Kai -$damage HP (${JOHN_DOE_ATTACK_PERCENT}% Max HP; $after/$targetMaxHp)."
            targetId == KAI_ID ->
            ...
            c.entityKey == JOHN_DOE_KEY ->
              "Jane Doe tấn công ${target.name}: -$damage HP (${JOHN_DOE_ATTACK_PERCENT}% Max HP; $after/$targetMaxHp)."
      ...
      if (incomingRoll < enemyChance) {
        val damage = janeDoeDamage(resolvedState, c.playerMaxHp, JOHN_DOE_ATTACK_PERCENT)
        ...
        log += "Jane Doe tấn công: Kai -$damage HP (${JOHN_DOE_ATTACK_PERCENT}% Max HP; ${c.playerHp}/${c.playerMaxHp})."
      } else {
        log += "Jane Doe không xuyên được thế phòng thủ/di chuyển của Kai."
      }
    ...
    if (c.entityKey == JOHN_DOE_KEY && c.entityHp > 0) {
      val targets = entityCombatActionTargets(resolvedState)
    ...
    if (c.entityKey == JOHN_DOE_KEY && c.eventCounter % JOHN_DOE_POISON_INTERVAL_TURNS == 0 &&
      ...
      log += "Jane Doe gây Poison sau ${JOHN_DOE_POISON_INTERVAL_TURNS} lượt: proc ${JOHN_DOE_POISON_CHANCE_PERCENT}% thành công; đánh dấu riêng ${affected.size} thành viên ACTIVE."
    }
    if (c.entityKey == JOHN_DOE_KEY && c.eventCounter % JOHN_DOE_STUN_INTERVAL_TURNS == 0 &&
      ...
      log += "Jane Doe Stun check: cổng ${JOHN_DOE_STUN_GATE_PERCENT}% và proc ${JOHN_DOE_STUN_PROC_PERCENT}% cùng thành công; mục tiêu bị Stun 1 lượt kế tiếp."
    }
  ...
  private fun janeDoeLilith(state: GameState): Boolean = state.metadata[JANE_DOE_LILITH_KEY].equals("true", true)
  ...
  private fun defeatLootNarration(state: GameState, defeatId: String): String {
