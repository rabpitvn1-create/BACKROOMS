...
- Tuyến AI cuối cùng: **Gemini K1 → K2 → K3 → K4 → K5 → K6 → HAKU → LUNA → controlled failure**.
- Bộ Gemini credential dùng naming canonical từ project `Hua-s-Family`: `GEMINI_API_KEY`, `GEMINI_API_KEY_2` ... `GEMINI_API_KEY_6`; không hard-code hoặc ghi secret vào log.
...
- Log TXT tiếp tục redaction toàn bộ HAKU/LUNA/Gemini credentials, nay bao phủ cả Gemini K6.
- CI có live smoke cho cả sáu Gemini credential trước khi chấp nhận provider authority mới.
